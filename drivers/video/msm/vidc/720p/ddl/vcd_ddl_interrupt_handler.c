...
static u32 ddl_header_done_callback(struct ddl_context *ddl_context)
{
	...
	ddl_calculate_stride(&decoder->frame_size,
			...
			decoder->codec.codec);
       	// 20110416 jinkyu.seong@lge.com MS910: fix H.264 streaming issue for DCTS (SR#00499845) [START]
       	if (decoder->buf_format.buffer_format == VCD_BUFFER_FORMAT_TILE_4x2) {
       	               ...
       	               decoder->frame_size.scan_lines = DDL_TILE_ALIGN(decoder->frame_size.height,DDL_TILE_ALIGN_HEIGHT);
       	}
       	// 20110416 jinkyu.seong@lge.com MS910: fix H.264 streaming issue for DCTS (SR#00499845) [END]
