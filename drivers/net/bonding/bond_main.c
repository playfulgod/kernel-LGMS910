...
#ifdef LG_DATA_PRIVATE_SOURCE_IP_FILTER
	{
		struct iphdr	*skb_ip_hdr;
		struct ethhdr	*skb_eth_hdr;
		//struct tcphdr	*skb_tcp_hdr;
		uint8_t *src_addr_array;
		uint8_t *dst_addr_array;

			skb_ip_hdr		= ip_hdr(skb);
			skb_eth_hdr		= eth_hdr(skb);
			
			if (skb_ip_hdr == NULL || skb_eth_hdr == NULL) {
				
				goto last;
			}
			
			src_addr_array	= (uint8_t *)&(skb_ip_hdr->saddr);
			dst_addr_array	= (uint8_t *)&(skb_ip_hdr->daddr);

			/* If ipv4 packet */
			if (skb_eth_hdr->h_proto == htons(ETH_P_IP) && skb_ip_hdr->version == 4) {
				/* If it start with 192.x.x.x, drop */
				if ((src_addr_array[0] == 192) && (dst_addr_array[0] != 192))  {
					
					/* calling function will free skb */
					printk(KERN_ERR "%s drop ip packet src %d.%d.%d.%d, " 
							"dst %d.%d.%d.%d, protocol 0x%x\n",
							bond->dev->name,
							src_addr_array[0],
							src_addr_array[1],
							src_addr_array[2],
					...
					/* TODO if tcp display tcp flags
					   skb_tcp_hdr = tcp_hdr(skb);
					 */
...
#endif	/* LG_DATA_PRIVATE_SOURCE_IP_FILTER */
