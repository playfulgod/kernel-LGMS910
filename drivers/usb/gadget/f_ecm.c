...
static char ecm_serial_number[256] = "LGANDROIDVS910";
static const char lg_serial_number[256] = "LGANDROIDVS910";
#endif
...
/* 2010.1.18 jaeho.cho exchange ecm Mac address string index */
	...
	snprintf(ecm_serial_number, sizeof(ecm_serial_number),
		"%02X%02X%02X%02X%02X%02X",
		ethaddr[0], ethaddr[1], ethaddr[2],
		ethaddr[3], ethaddr[4], ethaddr[5]);
#endif
...
{
    if(ecm_serial_number == NULL)
    {
      dest = lg_serial_number;
    }
    else
    {
      dest = ecm_serial_number;
    }
}
#endif
