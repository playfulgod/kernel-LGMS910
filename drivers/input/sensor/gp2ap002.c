...
static unsigned int gp2ap_debug_mask = GP2AP_DEBUG_DEV_STATUS | \
					GP2AP_DEBUG_DEV_DEBOUNCE | GP2AP_DEBUG_GEN_INFO;
...
#else
static int prox_i2c_read(u8 reg, u8 pval)
{
	int ret;
	int status;

	if (gp2ap_pdev->client == NULL) { 	/* No global client pointer? */
		printk("client is null\n");
		return -1;
	}

	if ((ret = i2c_smbus_read_byte_data(gp2ap_pdev->client, reg | pval)) >= 0) {
		pval = ret;
		status = 0;
	} else {
		status = -EIO;
		printk("fail to read(reg=0x%x,val=0x%x)\n", reg,pval);
	}

	return status;
}

static int prox_i2c_write(u8 reg, u8 val, u8 intclr)
{
	int ret;
	int status;
	...
	ret = i2c_smbus_write_byte_data(gp2ap_pdev->client, reg | intclr, val);
	if (ret == 0) {
		status = 0;
	} else {
		status = -EIO;
		printk("fail to write(reg=0x%x,val=0x%x)\n", reg, val);
	}
...
gp2ap_device_initialise(void)
{
	...
	hys = (gp2ap_pdev->methods) ? 0x40 : 0xC2;
...
static int prox_power_set(unsigned char onoff)
{
        ...
        int ret = 0;
        struct vreg *L20_vreg = vreg_get(0, "gp13");
		struct vreg *sensor_vreg;

	sensor_vreg = vreg_get(0, "gp2");
	if (IS_ERR(sensor_vreg)) {
        ...
        if (onoff == 1) {
				ret = vreg_set_level(sensor_vreg, 3000);
                ret = vreg_enable(sensor_vreg);
                if (ret != 0) {
                        printk("L11_vreg failed.\n");
                        return -1;
                }else {
                printk("motion v3.0 on \n");
                }

				
        } else if (onoff == 2) {
                ret = vreg_set_level(L20_vreg, 2700);
                ret = vreg_enable(L20_vreg);
                if (ret != 0) {
                        printk("L20_vreg failed.\n");
                        return -1;
                }
				else {
				printk("proxi v2.7 on \n");
				}
				
        } else if (onoff == 10){
			ret = vreg_set_level(sensor_vreg, 0);
            ret = vreg_disable(sensor_vreg);
        }
		else if (onoff == 20) { 
			ret = vreg_set_level(L20_vreg, 0);
            ret = vreg_disable(L20_vreg);
		}
		else {
			ret = vreg_set_level(L20_vreg, 0);
            ret = vreg_disable(L20_vreg);
...
//	prox_power_set(1);
	...
	gp2ap_pdev->methods = PROX_TMODE_NORMAL;
	gp2ap_pdev->last_vout = -1;
	gp2ap_pdev->debounce = 0;
	gp2ap_pdev->asd = 0;		/* disable */
	gp2ap_pdev->cycle = 0;		/*  8ms */
...
/*eycho - ok*/
	...
	gp2ap_pdev->sw_mode = PROX_STAT_SHUTDOWN;
//#endif
/*eycho - not ok*/
...
gp2ap_suspend(struct i2c_client *i2c_dev, pm_message_t state)
{
	...
	if (pdev->methods) 	/* Interrnupt mode */
		ret	= prox_i2c_write(GP2AP_REG_OPMOD, (GP2AP_ASD_SHIFT(pdev->asd) | 0x02), GP2AP_NO_INTCLEAR);
	else				/* Normal mode */
		ret	= prox_i2c_write(GP2AP_REG_OPMOD, (GP2AP_ASD_SHIFT(pdev->asd) | 0x00), GP2AP_NO_INTCLEAR);
...
#if 0
	ret = prox_i2c_write(GP2AP_REG_GAIN, 0x08, GP2AP_NO_INTCLEAR);
	hys = (pdev->methods) ? 0x40 : 0xC2;
	ret = prox_i2c_write(GP2AP_REG_HYS, hys, GP2AP_NO_INTCLEAR);
	ret = prox_i2c_write(GP2AP_REG_CYCLE, gp2ap_cycle_table[pdev->cycle].val, GP2AP_NO_INTCLEAR);

	if (pdev->methods) 	/* Interrnupt mode */
		ret = prox_i2c_write(GP2AP_REG_OPMOD, (GP2AP_ASD_SHIFT(pdev->asd) | 0x03), GP2AP_NO_INTCLEAR);
	else				/* Normal mode */
		ret = prox_i2c_write(GP2AP_REG_OPMOD, (GP2AP_ASD_SHIFT(pdev->asd) | 0x01), GP2AP_NO_INTCLEAR);
#else
	/* set up registers according to VOUT output mode */
	ret = gp2ap_device_initialise();
	if (GP2AP_DEBUG_ERR_CHECK & gp2ap_debug_mask) {
