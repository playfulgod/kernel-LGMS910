...
#if defined (CONFIG_LGE_CHARGING)
/* LGE_CHNAGE
 * ADD THUNDERC feature to use VS740 BATT DRIVER IN THUNDERC
 * 2010-05-13, taehung.kim@lge.com
 */
...
//yunjeong.kang 201.06.03
	static char *pseudo_batt[] = {
		"NORMAL", "THM_PSEUDO","BATT_PSEUDO",
	};
	...
	if (off == POWER_SUPPLY_PROP_PSEUDO_BATT)
		...
		return sprintf(buf, "[%s] \nusage: echo [mode] [thm_only_mode] [ID] [therm] [temp] [volt] [cap] [charging] > pseudo_batt\n", pseudo_batt[value.intval]);
#else
	    return sprintf(buf, "[%s] \nusage: echo [mode] [thm_only_mode] [thm_valid] [therm] [temp] [volt] [cap] [charging] > pseudo_batt\n", pseudo_batt[value.intval]);
#endif
	return 0;
}
...
#ifdef CONFIG_LGE_BATTERY_ID
	if (sscanf(buf, "%d %d %d %d %d %d %d %d", &info.mode, &info.thm_only_mode, &info.id, &info.therm,
				&info.temp, &info.volt, &info.capacity, &info.charging) != 8)
#else
	if (sscanf(buf, "%d %d %d %d %d %d %d %d", &info.mode, &info.thm_only_mode, &info.thm_valid, &info.therm,
				&info.temp, &info.volt, &info.capacity, &info.charging) != 8)
#endif
	{
		if((info.mode == 1) ||(info.thm_only_mode == 1)) //pseudo mode
		{
#ifdef CONFIG_LGE_BATTERY_ID
			printk(KERN_ERR "usage : echo [mode][thm_only_mode] [ID] [therm] [temp] [volt] [cap] [charging] > pseudo_batt");//yunjeong.kang 201.06.03
#else
			printk(KERN_ERR "usage : echo [mode][thm_only_mode] [thm_valid] [therm] [temp] [volt] [cap] [charging] > pseudo_batt");
#endif
			goto out;
		}
	}
...
// BEGIN: 0010099 hyunjong.do@lge.com 2010-10-21
// ADD: 0010099: [Power] Charging current control by thermister on Kernel
#if 1//defined (CONFIG_MACH_MSM7X27_ALOHAV) || defined (CONFIG_MACH_MSM7X27_THUNDERC)
/* LGE_CHNAGE
 * ADD THUNDERC feature to use VS740 BATT DRIVER IN THUNDERC
 * 2010-05-13, taehung.kim@lge.com
 */
	/* LGE_CHANGES_S [woonghee.park@lge.com] 2010-02-09, [VS740] */
/* LGE_CHANGE_S [jaeho.cho@lge.com] 2011-06-01, don't remove this to provide for framewrok and if CONFIG_LGE_BATTERY_ID is undefined, set 2 for this variable */
