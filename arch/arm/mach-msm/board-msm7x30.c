...
#include "lge/lg_fw_diag_communication.h"
...
static int kr3dh_config_gpio(unsigned char config)
{
	...
	if (err != 0)
		printk("GPIO_ACCEL_INT request failed.\n");
...
/* sungmin.cho@lge.com 2010.12.31 Change 3-Axis sensor direction */
...
// ADD HALLIC youngcheon.seo@lge.com
...
static int bryce_gpio_slide_input_func(struct input_dev *input_dev,
...
#ifdef CONFIG_MACH_LGE_BRYCE
	,
#ifdef CONFIG_FB_MSM_MDDI_LGIT_WVGA
	&mddi_lgit_device,
#endif
	&bl_i2c_device,	
	&sensor_i2c_device,
...
#if defined (CONFIG_MMC_MSM_SDC1_SUPPORT) && defined (CONFIG_MACH_LGE_BRYCE) && defined (CONFIG_LTE)
/* CONFIG_MACH_LGE_BRYCE	daeok.kim	10.08.09
	For LTE SDIO Driver
*/
/* CONFIG_MACH_LGE_BRYCE	daeok.kim	10.10.06
...
// LGE_CHANGES_S [cheolsook.lee@lge.com] 2011-02-23 [MS910] Add bcm4329
#if defined(CONFIG_LGE_BCM432X_PATCH)
static unsigned int bcm432x_sdcc_wlan_slot_status(struct device *dev)
{
	printk(KERN_ERR "%s: CONFIG_BCM4329_GPIO_WL_RESET = %d \n",	__func__, CONFIG_BCM4329_GPIO_WL_RESET);
...
#endif     // end of CONFIG_LGE_BCM432X_PATCH
// LGE_CHANGES_E [cheolsook.lee@lge.com] 2011-02-23 [MS910] Add bcm4329
#endif
 ...
 */
#if defined (CONFIG_MACH_LGE_BRYCE) && (defined (CONFIG_WLAN) || defined (CONFIG_BT))
...
#ifdef CONFIG_MACH_LGE_BRYCE //START hyogook.lee@lge.com 2011-05-09 Restore VS910 LDO init config
/* ey.cho@lge.com    11.02.27
   START 0017240: [Touch] Changed initialise code that power on after power off  */
	touch_power(0);
/*   END 0017240: [Touch] Changed initialise code that power on after power off  */

	static struct vreg *ttt_vreg;

//START hyogook.lee@lge.com 20011-05-12 tempolary LDO ON , Need to retouch 
	ttt_vreg = vreg_get(NULL, "gp4"); //PERI -> Acc Senesor
	rc = vreg_set_level(ttt_vreg, 2600);
	vreg_enable(ttt_vreg);

		ttt_vreg = vreg_get(NULL, "gp7"); 
	rc = vreg_set_level(ttt_vreg, 1800); //SENSOR_IO -> Battery 
	vreg_enable(ttt_vreg);
//END hyogook.lee@lge.com 20011-05-12 tempolary LDO ON , Need to retouch

#else

	ttt_vreg = vreg_get(NULL, "gp4"); //PERI 
	rc = vreg_set_level(ttt_vreg, 2600);
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "wlan"); //TOUCH
	rc = vreg_set_level(ttt_vreg, 3000);
	vreg_enable(ttt_vreg);


	ttt_vreg = vreg_get(NULL, "mmc"); //MSM_SD
	rc = vreg_set_level(ttt_vreg, 2600);
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "ruim");
	rc = vreg_set_level(ttt_vreg, 2850); //RUIM
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp2");
	rc = vreg_set_level(ttt_vreg, 2600); //EARMIC
	vreg_enable(ttt_vreg);
	
	ttt_vreg = vreg_get(NULL, "gp6");
	rc = vreg_set_level(ttt_vreg, 2800); //LCD
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "rf");
	rc = vreg_set_level(ttt_vreg, 1800); //PROXI_IO
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp7");  
	rc = vreg_set_level(ttt_vreg, 1800); //SENSOR_IO
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp8");
	rc = vreg_set_level(ttt_vreg, 1200); //VDD_P0
	vreg_enable(ttt_vreg);


	ttt_vreg = vreg_get(NULL, "gp9");
	rc = vreg_set_level(ttt_vreg, 1800); //TOUCH
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp10");
	rc = vreg_set_level(ttt_vreg, 1800); //LCD
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp11");
	rc = vreg_set_level(ttt_vreg, 1800); //QFUSE
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp12");
	rc = vreg_set_level(ttt_vreg, 2200); //XOADC_REF
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp13");
	rc = vreg_set_level(ttt_vreg, 3000); //MOTOR
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp14");
	rc = vreg_set_level(ttt_vreg, 1100); //MPLL
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp15");
	rc = vreg_set_level(ttt_vreg, 2600); //TP
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "gp16");
	rc = vreg_set_level(ttt_vreg, 1200); //QTR_WCA_DIG
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "s4");
	rc = vreg_set_level(ttt_vreg, 2200); //RF
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "xo_out");
	rc = vreg_set_level(ttt_vreg, 2600); //PROXI
	vreg_enable(ttt_vreg);

	ttt_vreg = vreg_get(NULL, "wlan2");
	rc = vreg_set_level(ttt_vreg, 2850); // GPS , L19
	vreg_enable(ttt_vreg);

#endif //END hyogook.lee@lge.com 2011-05-09 Restore VS910 LDO init config
...
#if 1 //defined (CONFIG_MACH_LGE_BRYCE) && defined (CONFIG_LGE_BCM432X_PATCH)
#else
	bt_power_init();
#endif
