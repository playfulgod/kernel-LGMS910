...
struct adie_codec_action_unit testmode_speakerphone_rx_8KHz_osr256_actions[] =
{
	{ADIE_CODEC_ACTION_STAGE_REACHED, ADIE_CODEC_FLASH_IMAGE}, \
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x02, 0x02)}, \
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x02, 0x00)}, \
...
static struct adie_codec_hwsetting_entry speakerphone_testmode_lb_rx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = testmode_speakerphone_rx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(testmode_speakerphone_rx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile testmode_speakerphone_rx_profile = {
	.path_type = ADIE_CODEC_RX,
	.settings = speakerphone_testmode_lb_rx_settings,
	.setting_sz = ARRAY_SIZE(speakerphone_testmode_lb_rx_settings),
};

struct adie_codec_action_unit testmode_speakerphone_tx_8KHz_osr256_actions[] =
{	
	{ADIE_CODEC_ACTION_STAGE_REACHED, ADIE_CODEC_FLASH_IMAGE}, \
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x01)}, \
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x00)}, \
...
static struct adie_codec_hwsetting_entry speakerphone_testmode_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = testmode_speakerphone_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(testmode_speakerphone_tx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile testmode_speakerphone_tx_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = speakerphone_testmode_lb_tx_settings,
	.setting_sz = ARRAY_SIZE(speakerphone_testmode_lb_tx_settings),
};
...
/*[START] jy0127.jang@lge.com MS910_AUDIO_CHANGE 2011-04-07*/
/*added speaker phone loopback*/
void QTR_speakerphone_loopback_for_testmode(u32 loop)
{
	int trc;
	struct msm_afe_config afe_config;
	struct lpa_codec_config lpa_config;
...
*************************************************************************************/
   ...
   /*enable LPA clock*/
	...
	/* Set MI2S */
		...
		MM_INFO("configure ADIE RX path\n");
	/* Configure ADIE */
		trc = adie_codec_open(&testmode_speakerphone_rx_profile, &testmode_lb_rx_adie);
		if (IS_ERR_VALUE(trc))
			goto tm_rx_error_adie;
		adie_codec_setpath(testmode_lb_rx_adie, 8000, 256);
	/* Enable LPA codec*/
	...
	/*start adie streaming*/	
		...
		ADIE_CODEC_DIGITAL_ANALOG_READY);	
	/*Set amp for Headset Path*/
		set_amp_gain(7);  //for lge_snddev_hs_phone_amp_on
	/*Enable SideTone */
		adie_codec_enable_sidetone(testmode_lb_rx_adie, 0x01);
/************************************************************************************
*  Tx Path Settings
...
/*ADD 0011453: Front Mic & Back Mic Separate LoopBack Support*/		
	...
	/* Configure ADIE */
		...
		{
			MM_INFO("configure Front Mic ADIE TX path\n");
			trc = adie_codec_open(&testmode_speakerphone_tx_profile, &testmode_lb_tx_adie);
			if (IS_ERR_VALUE(trc))
				goto tm_tx_error_adie;
		}
		adie_codec_setpath(testmode_lb_tx_adie, 8000, 256);
